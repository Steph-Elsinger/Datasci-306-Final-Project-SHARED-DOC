...
tobacco_region |>
  ...
  labs(title = "Average US Adult Smoking Rates by Race and Region in 2023",
...
tobacco_region |>
  ...
  select(`Demographic`,
         ...
         `Disparity Value`) |>
  drop_na() |>
  slice_max(`Disparity Value`, n = 100) |>
  
  ggplot(aes(x = "", fill = Demographic)) +
  geom_bar(width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Share of Top 100 Smoking Disparity Values by Demographic in 2023",
       x = NULL, y = NULL,
       fill = "Demographic") +
       theme_void()
...
tobacco_region |> group_by(`Year`, region) |> 
  summarize(avg = mean(`Cigarette Use Prevalence % (Reference group)`, na.rm = T)) |>
  
  ggplot() + geom_line(aes(x = `Year`, 
                           y = avg, color = region)) +
    labs(title = "Average Smoking Rate by Reference Group's Region",
