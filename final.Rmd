...
```
```{r}
# Step 1: Convert to numeric (already done above)
...
# Step 3: Merge CONDITIONALLY with many-to-many allowed
merged <- income_clean %>%
  inner_join(m_health_clean,
             by = c("State", "Year"),
             relationship = "many-to-many")

# Step 4: Calculate avg smoking for each Income × MentalHealth pair
plot_df <- merged %>%
  group_by(Income, MentalHealth) %>%
  summarize(
    Avg_Smoking = mean(c(Income_Smoking, MH_Smoking), na.rm = TRUE),
    .groups = "drop"
  )

# Step 5: Plot one bar per pair
ggplot(plot_df,
       aes(x = interaction(Income, MentalHealth, sep = " | "),
           y = Avg_Smoking,
           fill = Income)) +
  geom_col() +
  labs(
    title = "Average Smoking Percent for Each Income × Mental Health Pair",
    x = "Income Group | Mental Health Group",
    y = "Average Smoking Percent (%)",
    fill = "Income Bracket"
  ) +
  theme_minimal() +
...
```{r}
# Convert smoking prevalence to numeric (as in your PDF)
m_health_numeric <- m_health %>%
  mutate(
    `Cigarette Use Prevalence % (Focus group)` =
...
ggplot(
  ...
  aes(
    ...
    color = `Comparing (Focus group)`  # mental health category
  )
) +
  geom_point(alpha = 0.6) +
  geom_smooth(se = FALSE) +   # optional trend lines
  labs(
    title = "Smoking Prevalence Over Time by Mental Health Group",
    x = "Year",
