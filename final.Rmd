...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("year_A", "Year", value = 2026, min = 2023),
      ...
      numericInput("year_B", "Year", value = 2026, min = 2023)
    ),
...
server <- function(input, output, session) {
  ...
  # ----- NEW ratio text (raw numeric ratio, clean wording) -----
  ...
  output$regionTrendPlot <- renderPlot({
    tobacco_clean %>%
      ...
      summarize(
        avg = mean(cig_prev, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      ggplot(aes(x = Year, y = avg, color = region)) +
      geom_line(size = 1.2) +
      labs(
        title = "Average Smoking Rate Over Time by Region",
        y = "Average Cigarette Smoking Rate (%)",
        x = "Year",
        color = "Region"
      ) +
