...
plot_df <- merged %>%
  mutate(CombinedSmoking = (Income_Smoking + MH_Smoking) / 2) %>%   # <--- KEY FIX
  group_by(Income, MentalHealth) %>%
  summarize(avg_combined = mean(CombinedSmoking, na.rm = TRUE),
            .groups = "drop")
...
ggplot(plot_df,
  ...
  labs(
    ...
    x = "Income Bracket (Focus Group)",
    y = "Avg Combined Cigarette Smoking Prevalence (%)",
    fill = "Mental Health (Focus Group)"
  ) +
  theme_minimal() +
...
```{r}
options(scipen = 999)
model <- lm(`Cigarette Use Prevalence % (Focus group)` ~ Year + `Comparing (Focus group)` + region,
            data = tobacco_region)
